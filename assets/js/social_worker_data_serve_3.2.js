$(document).ready(function () {
  ...
  $("#sendSocialWorkerData").on("click", function (e) {
    ...
    var bankAccountShebaNumber = $(
      "#social_worker_bank_account_sheba_number"
    ).val();
    var bankAccountCardNumber = $(
      "#social_worker_bank_account_card_number"
    ).val();
  ...
  function deactivateSW(id) {
    ...
    $.ajax({
      ...
      contentType: false,
      beforeSend: function () {
        var confirmDeactivate = confirm(
          "You are about to DEACTIVATE the penel user #" +
            id +
            ".\nAre you sure?"
        );
      ...
      statusCode: {
        400: function () {
          ...
          var valid = /^[0-9]*$/;
          var newSwId = prompt(
            "مددکار دارای کودک می‌باشد، ابتدا باید کودکان او به مددکاری دیگر انتقال یابند.\nلطفا آی دی مددکار جدید را وارد نمایید:"
          );
          if (newSwId) {
            if (!newSwId.match(valid)) {
              alert(
                "آی دی تنها می‌تواند شامل اعداد باشد.\n مقدار وارد شده: (" +
                  newSwId +
                  ")"
              );
            } else {
  ...
  $("#socialWorkerList").on("click", ".deleteBtn", function (e) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        return confirm(
          "You are about to DELETE the penel user.\nAre you sure?"
        );
      },
      success: function (data) {
        alert("Success\n" + JSON.stringify(data.message));
