$(document).ready(function(){
        ...
        success: function (data) {
          ...
          $.each(childData, function (key, value) {
            ...
            for (var i = 1; i < keys.length; i++) {
              ...
              if (keys[i] == 'cityId') {
                if (value[keys[i]] == '135129') {
                  value[keys[i]] = 'Tehran';
                }
                if (value[keys[i]] == '134664') {
                  value[keys[i]] = 'Karaj';
                }
              }
              ...
              if (keys[i] == 'birthPlaceId') {
                ...
                if (value[keys[i]] == '135131') {
                  value[keys[i]] = 'Torbat-e Jām';
                }
              }

              if (keys[i] == 'nationalityId') {
                if (value[keys[i]] == '103') {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.each(childData, function (key, value) {
          var query = '';
          query += `<option value=${value['id']}>${value['generatedCode']} - ${
            value['firstName']
          } ${value['lastName']} - ${getAge(value['birthDate'])}</option>`;
    ...
    $('#sendChildData').on('click', function (e) {
      ...
      var cityId = parseInt($('#child_city').val());
      ...
      // Nullable
      var nationalityId = parseInt($('#child_nationality').val());
      var address = $('#child_address').val();
      var birthPlaceId = parseInt($('#child_birthplace').val());
      ...
      if (nationalityId) {
        form_data.append('nationalityId', nationalityId);
      }
      if (address) {
        form_data.append('address', address);
      }
      if (birthPlaceId) {
        form_data.append('birthPlaceId', birthPlaceId);
      }
      if (familyCount) {
      ...
      if (education || school_type) {
        form_data.append(
          'education',
          education == '-2' ? '-' + school_type + '2' : school_type + education
    ...
    $('#childList').on('click', '.editBtn', function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $('#child_nationality').val(data['nationalityId']).change();
          $('#child_city').val(data['cityId']).change();
          ...
          $('#child_birthplace').val(data['birthPlaceId']).change();
    ...
    $('#editChildData').on('click', function (e) {
      ...
      if (education || school_type) {
        form_data.append(
          'education',
          education == '-2' ? '-' + school_type + '2' : school_type + education
        ); // TODO: because "int(-2)" cannot be stored. temporarily solution!
      }
      if (housingStatus) {
        form_data.append('housingStatus', housingStatus);
      }
